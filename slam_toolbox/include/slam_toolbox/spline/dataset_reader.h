...
public:
  ...
  static void load_gt_file(std::string path, std::map<double, Eigen::Matrix<double, 17, 1>> &gt_states) {
   ...
   //   PRINT_ERROR(RED "ERROR: %s\n" RESET, path.c_str());
      std::cout << "Error: Uable to open groundtruth file ... " << std::endl;
    ...
    while (std::getline(file, line)) {
      ...
      while (getline(s, field, ',')) {
        ...
        if (i > 16) {
          ...
          // PRINT_ERROR(RED "ERROR: %s\n" RESET, line.c_str());
          std::cout << "Error: invalid groundtruth line, too long!" << std::endl;
  ...
  static bool get_gt_state(double timestep, Eigen::Matrix<double, 17, 1> &imustate,
    ...
    if (gt_states.empty()) {
      ...
      std::cout << "Ground truth data loaded is empty, make sure you call load before asking for a state." << std::endl;
      return false;
    }
    ...
    if (gt_states.find(timestep) == gt_states.end()) {
      ...
      std::cout << "Uable to find " << timestep << " timestamp in GT file, wrong GT file loaded???" << std::endl;
      return false;
    }
...
//      PRINT_ERROR(RED "ERROR: %s\n" RESET, path.c_str());
      std::cout << "ERROR: Unable to open simulation trajectory file..." << std::endl;
      std::cout << "ERROR: " << path << std::endl;
      std::exit(EXIT_FAILURE);
    }
    ...
    std::cout << "loaded trajectory: " << base_filename << std::endl;
    ...
    if (traj_data.empty()) {
      ...
      // PRINT_ERROR(RED "ERROR: %s\n" RESET, path.c_str());
      std::cout << "Error: Could not parse any data from the file !!" << std::endl;
      std::cout << "Error: " << path << std::endl;
      std::exit(EXIT_FAILURE);
    }
  }
